...
def main():
    ...
    proof = user.keypair.sign(hashlib.new(HASH_ALGO, b"register" + user.entity_id).digest())
    r = rpc("register_entity", {
        "entity_id": user.entity_id.hex(),
        "public_key": user.public_key.hex(),
        "registration_proof": proof.serialize(),
        "sponsor_id": gen.entity_id.hex(),  # genesis sponsors the REGISTRATION_FEE
    })
    ok("register new user (sponsored by genesis)", r)
    print("  Waiting for registration to confirm...")
    ...
    ok("fund user with 5000 tokens", r)
    nonce += 1
    print("  Waiting for transfer to confirm...")
    wait_block(start_h + 2)

    # ── 3. New user sends messages ──
    sec("3. New user sends messages")
    un = refresh(user)
    ...
    proof3 = w3.keypair.sign(hashlib.new(HASH_ALGO, b"register" + w3.entity_id).digest())
    r = rpc("register_entity", {
        "entity_id": w3.entity_id.hex(),
        "public_key": w3.public_key.hex(),
        "registration_proof": proof3.serialize(),
        "sponsor_id": gen.entity_id.hex(),
    })
    ok("register wallet3", r)
    print(f"  Wallet3: {encode_address(w3.entity_id)}")
    print("  Waiting for wallet3 registration to confirm...")
    wait_block(start_h + 5)

    tx = create_transfer_transaction(gen, w3.entity_id, amount=1000, nonce=nonce, fee=MIN_FEE)
    r = rpc("submit_transfer", {"transaction": tx.serialize()})
    ok("genesis -> wallet3 (1000)", r)
    nonce += 1
