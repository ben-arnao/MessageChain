...
class TestInflation(unittest.TestCase):
    ...
    def test_halving_reduces_reward(self):
        ...
        not the halving schedule."""
        reward_early = self.tracker._calculate_legacy_block_reward(0)
        reward_after_halving = self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL)
    ...
    def test_four_meaningful_halvings(self):
        ...
        Pre-Tier-47 invariant — see test_halving_reduces_reward.
        """
        self.assertEqual(self.tracker._calculate_legacy_block_reward(0), 16)
        self.assertEqual(self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL), 8)
        self.assertEqual(self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL * 2), 4)
        self.assertEqual(self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL * 3), 4)
        self.assertEqual(self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL * 4), 4)
        self.assertEqual(self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL * 5), 4)

    def test_reward_never_reaches_zero(self):
        """Under the legacy schedule, reward stays ≥ floor after all halvings.

        Pre-Tier-47 invariant — see test_halving_reduces_reward.
        """
        reward = self.tracker._calculate_legacy_block_reward(HALVING_INTERVAL * 100)
