...
class TestTransferOnChain(unittest.TestCase):
    ...
    def test_validate_transfer_valid(self):
        ...
        tx = create_transfer_transaction(
            self.genesis, self.recipient.entity_id, 100, nonce=nonce, fee=1,
        )
    ...
    def test_validate_transfer_unknown_sender(self):
        ...
        tx = create_transfer_transaction(
            unknown, self.recipient.entity_id, 50, nonce=0, fee=1,
        )
        valid, reason = self.chain.validate_transfer_transaction(tx)
        self.assertFalse(valid)
    ...
    def test_validate_transfer_unknown_recipient(self):
        ...
        tx = create_transfer_transaction(
            self.genesis, unknown.entity_id, 50, nonce=nonce, fee=1,
        )
        valid, reason = self.chain.validate_transfer_transaction(tx)
        self.assertFalse(valid)
    ...
    def test_validate_transfer_wrong_nonce(self):
        ...
        tx = create_transfer_transaction(
            self.genesis, self.recipient.entity_id, 50, nonce=999, fee=1,
        )
        valid, reason = self.chain.validate_transfer_transaction(tx)
        self.assertFalse(valid)
    ...
    def test_transfer_in_block(self):
        ...
        amount = 100
        tx = create_transfer_transaction(
            self.genesis, self.recipient.entity_id, amount, nonce=nonce, fee=1,
        )
...
class TestReadMessages(unittest.TestCase):
    ...
    def test_get_recent_messages_returns_messages(self):
        ...
        tx = create_transaction(self.genesis, "Hello chain!", fee=1, nonce=nonce)
    ...
    def test_get_recent_messages_respects_count(self):
        ...
        for i in range(3):
            ...
            tx = create_transaction(self.genesis, f"Message {i}", fee=1, nonce=nonce)
            block = self.chain.propose_block(consensus, self.genesis, [tx])
