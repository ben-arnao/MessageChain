...
class TestSqrtDefaultDelegation(unittest.TestCase):
    """Non-delegating users' weight is distributed by sqrt(stake) default."""

    def setUp(self):
        self.tracker = GovernanceTracker()
        self.supply = SupplyTracker()

    def test_default_delegation_uses_sqrt(self):
        """Users without explicit delegation distribute weight by sqrt(stake)."""
        v_big = b"\x01" * 32    # big validator
        v_small = b"\x02" * 32  # small validator
        passive = b"\x03" * 32  # user who never delegated

        self.supply.staked[v_big] = 10000
        self.supply.staked[v_small] = 100
        self.supply.staked[passive] = 1000

        tx = ProposalTransaction(
            proposer_id=v_big, title="Test", description="Test",
            timestamp=1.0, fee=1000, signature=None,
        )
        tx.tx_hash = tx._compute_hash()
        self.tracker.add_proposal(tx, block_height=0, supply_tracker=self.supply)
        pid = tx.proposal_id
        ...
        from messagechain.governance.governance import VoteTransaction
        # Both validators vote yes
        for vid in [v_big, v_small]:
            vote = VoteTransaction(
                voter_id=vid, proposal_id=pid, approve=True,
                timestamp=1.0, fee=100, signature=None,
            )
            vote.tx_hash = vote._compute_hash()
            self.tracker.add_vote(vote, current_block=0)
        ...
        # passive's 1000 stake should be split by sqrt weighting:
        # sqrt(10000) = 100, sqrt(100) = 10, total_sqrt = 110
        # v_big gets 1000 * 100/110 = 909
        # v_small gets 1000 * 10/110 = 90 (remainder)
        # Both voted yes, so all of passive's weight goes to yes
        # Total: v_big(10000) + v_small(100) + passive_distributed(1000) = 11100
        self.assertEqual(total_weight, 11100)
        self.assertEqual(yes_weight, 11100)

    def test_default_only_distributes_to_voters(self):
        """Default delegation only distributes to validators who actually voted."""
        v1 = b"\x01" * 32
        v2 = b"\x02" * 32  # doesn't vote
        passive = b"\x03" * 32

        self.supply.staked[v1] = 1000
        self.supply.staked[v2] = 1000
        self.supply.staked[passive] = 500
        ...
        # Only v1 votes
        vote = VoteTransaction(
            voter_id=v1, proposal_id=pid, approve=True,
            timestamp=1.0, fee=100, signature=None,
        )
        vote.tx_hash = vote._compute_hash()
        self.tracker.add_vote(vote, current_block=0)
        ...
        yes_weight, total_weight = self.tracker.tally(pid)
        # v1 gets default weight from both v2 (1000) and passive (500)
        # since neither voted and neither explicitly delegated
        self.assertEqual(yes_weight, 1000 + 1000 + 500)
