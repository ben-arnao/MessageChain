...
class TestCompactBlockFilters(unittest.TestCase):
    ...
    def test_filter_creation_for_block(self):
        ...
        tx = create_transaction(entities[0], "filter test", nonce=0, fee=MIN_FEE)
    ...
    def test_filter_matches_included_entity(self):
        ...
        tx = create_transaction(entities[0], "match test", nonce=0, fee=MIN_FEE)
        block = _propose_and_add(chain, consensus, entities[0], txs=[tx])
    ...
    def test_filter_does_not_match_absent_entity(self):
        ...
        tx = create_transaction(entities[0], "no match", nonce=0, fee=MIN_FEE)
        block = _propose_and_add(chain, consensus, entities[0], txs=[tx])
    ...
    def test_filter_serialization_roundtrip(self):
        ...
        tx = create_transaction(entities[0], "serialize test", nonce=0, fee=MIN_FEE)
        block = _propose_and_add(chain, consensus, entities[0], txs=[tx])
...
class TestMempoolPersistence(unittest.TestCase):
    ...
    def test_mempool_save_and_load(self):
        ...
        mempool = Mempool()
        tx1 = create_transaction(entities[0], "persist me 1", nonce=0, fee=2)
        tx2 = create_transaction(entities[0], "persist me 2", nonce=1, fee=3)
    ...
    def test_mempool_load_skips_expired(self):
        ...
        mempool = Mempool()
        tx = create_transaction(entities[0], "will expire", nonce=0, fee=2)
