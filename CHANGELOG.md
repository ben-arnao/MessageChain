...
MessageTransaction whose `sender_id` equals `voter_id`.  Pre-
activation blocks keep their original admission outcomes — historical
state must continue to apply under the rules in force when each
block was produced.
...
* `messagechain/storage/chaindb.py` — new `get_message_author`
  helper resolves `tx_hash → sender_id` via the existing
  ...
  fails, or the in-block tx is not a MessageTransaction.
...
`sender_id`, so resolving authorship is read-only against the
existing `tx_locations` index.
