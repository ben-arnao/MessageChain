...
- [x] **Hot signing key stored in an operator-managed secrets backend, not plaintext on disk.** Operator choice — HSM, cloud secrets manager, Vault, etc. The generic hand-off is: a systemd `ExecStartPre` fetches the hex key from the backend to a `tmpfs` path (e.g. `/dev/shm/mc-key`), then `server.py --keyfile /dev/shm/mc-key` reads it. Key never touches persistent disk. Backend-specific glue lives on the VM only — intentionally not checked into this repo to stay provider-agnostic.
- [x] **Hot key access logged to an immutable audit sink.** Whatever backend the operator picks (KMS, Vault, HSM), each key fetch must land in an append-only audit log.
...
- **2026-04-18 — Hot signing key removed from persistent disk.** Plain-file keyfile replaced by a tmpfs path fed from an operator-managed secrets backend at service start. Backend-specific glue (the `ExecStartPre` snippet that populates `/dev/shm/mc-key`) is installed on the VM but deliberately NOT committed to this repo — staying provider-agnostic in git. The server code path remains the same generic `--keyfile <path>` argument; only the path changes.
