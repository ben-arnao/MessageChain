...
class Server:
    ...
    def _rpc_register_entity(self, params: dict) -> dict:
        ...
        try:
            ...
            # Sponsor — a registered entity that pays the REGISTRATION_FEE on
            # behalf of the brand-new entity (which has zero balance).  Can be
            # omitted iff REGISTRATION_FEE is 0 (devnet).
            sponsor_hex = params.get("sponsor_id", "")
            sponsor_id = parse_hex(sponsor_hex) if sponsor_hex else b""
            if sponsor_id is None:
                return {"ok": False, "error": "Invalid hex in sponsor_id"}
