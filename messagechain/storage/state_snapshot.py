...
# v10: added treasury_spend_rolling_debits — rolling-window list of
...
#     tuples, appended after lottery_prize_pool.
...
# v10: treasury per-spend rolling-window debit list — drives the
...
def serialize_state(blockchain) -> dict:
    ...
    return {
        ...
        # v10: treasury per-spend rolling-window debit list.  List
...
def compute_state_root(snapshot: dict) -> bytes:
    ...
    section_roots: dict[bytes, bytes] = {
        ...
        # v10: treasury per-spend rolling-window debit list.  Each
...
def encode_snapshot(snap: dict) -> bytes:
    ...
    # v10: treasury per-spend rolling-window debit list, a variable-
    # length list of (block_height, amount) tuples.  Strictly
    # appended after the v9 scalar so a v9 blob is a strict prefix
    # of a v10 blob through the end of v9's final field.  Bounded
    # entry count: pre-cap-tighten only RETUNE-era spends accumulate
    # (zero pre-activation); post-tighten at most ~525 spends/year
    # given the 100-block epoch × 1-spend-per-epoch worst case.
...
def decode_snapshot(blob: bytes, max_bytes: int | None = None) -> dict:
    ...
    off += 8
    # v10+: treasury per-spend rolling-window debit list.  Always
    # present on v10+ blobs.  Pre-v10 blobs cannot reach here — the
